  ...
  return (
    <>
      ...
      >
        ...
        {user && (
          <div className="order-first md:order-last flex items-center gap-3 px-4 mb-6 md:mb-0 md:mt-6 md:pt-6 md:border-t md:border-surface-container">
            {user.imageUrl ? (
          ...
          {STUDENT_NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                onClick={onClose}
                className={cn(
                  "flex items-center gap-3 px-4 py-4 md:py-3 rounded-xl transition-colors duration-200 font-medium text-base md:text-sm",
                  isActive
...
function NotificationBellFallback() {
  return (
    <div
      aria-hidden
      className="w-full flex items-center gap-3 px-4 py-4 md:py-3 rounded-xl text-on-surface-variant font-medium text-base md:text-sm"
    >
      <Icon name="notifications" className="text-2xl md:text-xl" />
      <span>通知</span>
    </div>
  );
}
