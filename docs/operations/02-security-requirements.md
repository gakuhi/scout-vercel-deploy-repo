...
| job_postings | 自社メンバー: 全求人 / 学生: published のみ | 自社メンバー（is_verified = true） | 自社メンバー | 不可（論理削除） |
| chat_messages | スカウト当事者のみ | 当事者 かつ scouts.status = accepted | read_at のみ相手側 | 不可 |
| notifications | 本人宛のみ | システムのみ | is_read / read_at のみ本人 | 不可 |
| notification_settings | 本人 | 本人 | 本人 | 不可 |
| events | 公開イベント: 全ユーザー / 自社イベント: 自社メンバー | 企業メンバー（is_verified = true）/ 運営はService Role | 自社メンバー / 運営はService Role | 不可（論理削除） |
| event_registrations | 本人の申し込み / イベント主催企業のメンバー | 学生本人 | ステータス: 学生（キャンセル）/ 企業（確認・出席記録） | 不可 |
| anonymous_visits | 全拒否（Service Role のみ） | 全拒否（Service Role のみ） | 全拒否（Service Role のみ） | 全拒否（Service Role のみ） |
