...
| タグ | 型 | 抽出ロジック | 備考 |
|---|---|---|---|
| 興味業界 Top5 | industry_category[5] | 面接練習の志望業界 + 企業分析の調査企業の業界 + ES/志望動機の提出先企業の業界を集計し、出現頻度順に上位5つ | 順序に意味がある（1位が最も関心が高い） |
| 興味職種 | job_category[] | ガクチカ・ES・面接回答・履歴書の内容から Claude が推定 | 複数可。行動データに明示的な職種選択がないため Claude 推定 |
...
-- D. 興味タグ（行動データから自動抽出）
interested_industries       industry_category[5]  -- 興味業界 Top5（順序 = 関心度順）
interested_job_types        job_category[]         -- 興味職種（Claude 推定）
...
```json
{
  ...
  "interested_industries": ["it_software", "consulting", "finance", "internet_web", "manufacturing"],
  "interested_job_types": ["engineer", "planning"],
  "score_confidence": 75
}
```
...
| 希望業界 | desired_industries industry_category[] | 学生入力 or 行動データから自動付与 |
| 希望職種 | desired_job_types job_category[] | 同上 |
| 希望勤務地 | desired_locations region[] | 学生入力 |
...
CREATE TYPE job_category AS ENUM (
  'engineer_it',       -- ITエンジニア（SE・Web開発・インフラ等）
  'engineer_other',    -- 技術職（機械・電気・化学・生産技術等）
  'designer',          -- デザイナー
  'sales',             -- 営業
  'marketing',         -- マーケティング
  'planning',          -- 企画・事業開発
  'corporate',         -- コーポレート（経理・人事・法務）
  'consultant',        -- コンサルタント
  'research',          -- 研究職
  'other'
);
