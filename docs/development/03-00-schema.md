...
**本プロジェクトのテーブルは原則 `public` スキーマに配置する。** 例外として、クライアントからのアクセスを完全に遮断するテーブル（`audit_logs` 等）は `internal` スキーマに配置する。

| スキーマ | 管理者 | 用途 |
|---|---|---|
| `internal` | アプリ開発者 | クライアントからアクセスさせないテーブルを配置（監査ログ等） |

- PostgreSQL のスキーマはネスト（入れ子）できないフラットな構造
- `public` 内のテーブルはプレフィックスで論理グルーピングする（`student_*`, `company_*`, `scout_*`, `synced_*` など）
- MVP の規模では細かいスキーマ分割は不要。チームや権限が明確に分かれる規模になった場合に検討する
...
### ⚠️ 認証プロバイダー設定（マイグレー���ョン対象外）

認証プロバイダー（LINE Login、マジックリンク等）やMFAの設定はマイグレーションSQLでは管理できない。設定手順・運用ルールは [01-developer-setup.md](./01-developer-setup.md) のセクション6を参照。
...
| `scout_status` | `sent`, `accepted`, `declined`, `expired` | スカウトの状態遷移（既読判定は `read_at` で行う） |
| `academic_type` | `liberal_arts`, `science`, `arts`, `medical`, `sports`, `other` | 学問系統区分 |
...
| faculty | TEXT | | 学部 |
| department | TEXT | | 学科 |
| academic_type | academic_type | | 学問系統区分 |
...
| street | TEXT | | 番地以降 |
| phone | TEXT | | 代表電話番号 |
| is_public | BOOLEAN | DEFAULT false | 学生への公開フラグ（企業側が任意に切り替え） |
...
イベント駆動の通知レコード。学生にはLINE通知、企業担当者にはメール通知、両者にアプリ内通知を配信する。
...
| line_sent_at | TIMESTAMPTZ | | LINE通知送信日時（学生向け。NULL = 未送信 or LINE未連携） |
| email_sent_at | TIMESTAMPTZ | | メール通知送信日時（企業担当者向け。NULL = 未送信） |
| created_at | TIMESTAMPTZ | DEFAULT now() | |
...
| chat_message | BOOLEAN | DEFAULT true | チャット新着通知 |
| system_announcement | BOOLEAN | DEFAULT true | システムからのお知らせ |
| email_enabled | BOOLEAN | DEFAULT true | メール通知の一括ON/OFF |
| in_app_enabled | BOOLEAN | DEFAULT true | アプリ内通知の一括ON/OFF |
| updated_at | TIMESTAMPTZ | DEFAULT now() | |
...
| students | (academic_type) | 学問系統フィルター |
...
| academic_type | students.academic_type | 学問系統区分 |
...
※ 企業担当者は検索時にこの View を使用する。`students` テーブルへの直接アクセスはスカウト承諾後の実名・連絡先開示に使用する（RLS で承諾済みスカウトの学生のみ全カラム閲覧可）。
...
**RLS（Row Level Security）とは:** PostgreSQL の機能で、テーブルの各行に対して「誰が読み書きできるか」をポリシーとして定義する仕組み。Supabase ではクライアントからの全リクエストに RLS が適用されるため、アプリ層のバグがあってもDB層で不正アクセスをブロックできる。

**制御の粒度:**
- **行単位（RLS）**: どの行を SELECT / INSERT / UPDATE / DELETE できるかを制御する。RLS の基本機能
- **カラム単位**: RLS では制御できないため、用途に応じて View または GRANT を使い分ける

| | View | GRANT |
|---|---|---|
| 主な用途 | SELECT（読み取り制限） | UPDATE（書き込み制限） |
| できること | カラムの除外・加工・複数テーブルのJOIN | 特定カラムのみ操作を許可 |
| 本プロジェクトでの使用例 | `public_students`: 企業担当者に学生の実名・連絡先を隠しつつJOINでデータをまとめる | `GRANT UPDATE (status, read_at) ON scouts TO authenticated;`: 学生が更新できるカラムを制限 |
| SELECT制御に使わない理由 | — | JOINや加工ができず、カラム追加のたびに再設定が必要なため運用が煩雑 |
| UPDATE制御に使わない理由 | Updatable View の制約が多く複雑になるため | — |

以下のポリシー表で UPDATE 列に記載しているカラム制限は、GRANT によって実現する。

### 学生（student ロール）

**ビジネス要件:**
- 自分自身のデータのみ閲覧・編集できる。他の学生のデータは一切見えない
- 連携データ（synced_*）やAIプロフィールは閲覧のみ。編集はシステム側が行う
- 企業情報・求人・イベントは公開されているものだけ見える
- スカウトやチャットは自分が当事者のものだけ
...
| companies | is_public = true の企業のみ | — | — | — |
| job_postings | is_published = true かつ企業が is_public = true のみ | — | — | — |
...
| events | 公開イベント かつ企業が is_public = true（企業主催の場合）のみ | — | — | — |
| event_registrations | 自分の申し込みのみ | 公開イベントに申し込み | status（キャンセル）のみ | — |

### 企業担当者（company_owner / company_member ロール）

**ビジネス要件:**
- 「審査済み企業に所属していること」が全操作の前提
- 学生データはプロフィール公開中の学生のみ、個人特定情報を除いた View 経由で閲覧する
- 生の連携データ（synced_*）には直接アクセスしない。学生の活動実績はAIが統合した integrated_profile を通じて確認する
- スカウト承諾前は匿名状態。承諾後に実名・連絡先が開示される
- 自社のリソース（スカウト・求人・イベント・メンバー）は自社分のみ操作可能
- owner / member のロールによって権限の範囲が異なる
...
**ビジネス要件:**
- システム内部でのみ扱うべきデータ
- 流入経路トラッキング（anonymous_visits）はユーザーに見せる必要がない
- 通知の生成はサーバー側の判断で行うため、クライアントからの直接操作を許可しない
