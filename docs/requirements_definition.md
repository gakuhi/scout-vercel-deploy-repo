...
ロール設計・セッション管理・MFA要件の詳細は [セキュリティ要件書](security-requirements.md) を参照。
...
- **プライバシー・同意管理**: 学生がどのデータを公開するか制御する仕組み → [セキュリティ要件書](security-requirements.md) セクション7 で設計済み
...
| 個人情報保護法への対応 | 高 | [セキュリティ要件書](security-requirements.md) セクション7で対応済み（同意管理・公開範囲制御・退会時データ削除・保護法チェックリスト） |
...
| 企業アカウント審査フロー | 登録直後は is_verified=false。運営が審査完了するまで学生データ閲覧・スカウト送信不可。審査完了後にメール通知 | なりすまし防止。[セキュリティ要件書](security-requirements.md) 2.2 で定義 |
...
| RLS + 認証基盤 | 学生/企業の認証、テーブルごとのアクセス制御、MFA（owner/admin必須）、セッション管理 | セキュリティの根幹。詳細は [セキュリティ要件書](security-requirements.md) 参照 |
